...
enum TypeMessage
{
    ...
    ABORT, // Задача прервана(Главное окно записывает время прерывания задачи)
           // при отправке из родительского окна если name == all или  sheduler - останавливаются все задачи и планировщик
           // если  name == имя задачи - задача прерывается
    STATUS, // Просто информация???
